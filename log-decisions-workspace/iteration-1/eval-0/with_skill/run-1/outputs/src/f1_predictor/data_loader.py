...
# fastf1.Cache already persists raw HTTP responses to CACHE_DIR, but that
# doesn't stop us from re-issuing (and re-parsing) requests for the same
# year/round when feature engineering re-derives data for overlapping race
# windows within one training run. An in-memory memoization layer on top
# catches those same-process repeats for free -- see
# docs/decisions/001-in-memory-memoization-for-loader-functions.md.


def _memoize(fn):
    """lru_cache wrapper that returns a defensive copy of any DataFrame.

    Plain functools.lru_cache would hand every caller the *same* DataFrame
    object; if one caller mutated it in place, every other cached call would
    see the corruption. Returning a copy keeps the memoization invisible to
    callers.
    """
